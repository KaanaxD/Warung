...
describe("itemService", () => {
    ...
    describe("findItemById", () => {
        it("returns item when found", async () => {
            const item = { id: 1, nama: "test", kategori: "cat", updated_at: "2026-01-01", img_address: "img.webp" }
    ...
    describe("insertItem", () => {
        it("inserts and creates audit log", async () => {
            const newItem = { id: 1, nama: "test", kategori: "cat", updated_at: "", img_address: "img.webp" }
            mockPostItemQuery.mockResolvedValue([newItem])
            mockInsertLogs.mockResolvedValue([])

            const result = await (await itemService()).insertItem("admin", "test", "cat", "img.webp")

            expect(mockPostItemQuery).toHaveBeenCalledWith("test", "cat", "img.webp")
            expect(mockInsertLogs).toHaveBeenCalledWith("admin", 1, "CREATE", null, newItem)
            expect(result).toEqual(newItem)
        })

        it("inserts with null image", async () => {
            const newItem = { id: 2, nama: "test", kategori: "cat", updated_at: "", img_address: null }
            mockPostItemQuery.mockResolvedValue([newItem])
            mockInsertLogs.mockResolvedValue([])

            const result = await (await itemService()).insertItem("admin", "test", "cat", null)

            expect(mockPostItemQuery).toHaveBeenCalledWith("test", "cat", null)
            expect(result).toEqual(newItem)
        })
        ...
        it("throws 500 when insert returns empty array", async () => {
            ...
            await expect((await itemService()).insertItem("admin", "test", "cat", null)).rejects.toMatchObject({
                status: 500,
    ...
    describe("updateItem", () => {
        it("updates nama and kategori and creates audit log", async () => {
            const oldItem = { id: 1, nama: "old", kategori: "cat", updated_at: "", img_address: "old.webp" }
            const updatedItem = { id: 1, nama: "new", kategori: "dog", updated_at: "", img_address: "old.webp" }
            mockGetItemQuery.mockResolvedValue([oldItem])
            mockPutItemQuery.mockResolvedValue([updatedItem])
            mockInsertLogs.mockResolvedValue([])

            const result = await (await itemService()).updateItem("admin", 1, "new", "dog")

            expect(mockGetItemQuery).toHaveBeenCalledWith(1)
            expect(mockPutItemQuery).toHaveBeenCalledWith(1, "new", undefined, "dog")
            expect(mockInsertLogs).toHaveBeenCalledWith("admin", 1, "UPDATE NAMA_ITEM KATEGORI", oldItem, updatedItem)
            expect(result).toEqual(updatedItem)
        })

        it("updates with image", async () => {
            const oldItem = { id: 1, nama: "old", kategori: "cat", updated_at: "", img_address: "old.webp" }
            const updatedItem = { id: 1, nama: "new", kategori: "cat", updated_at: "", img_address: "new.webp" }
            mockGetItemQuery.mockResolvedValue([oldItem])
            mockPutItemQuery.mockResolvedValue([updatedItem])
            mockInsertLogs.mockResolvedValue([])

            await (await itemService()).updateItem("admin", 1, "new", "cat", "new.webp")

            expect(mockInsertLogs).toHaveBeenCalledWith("admin", 1, "UPDATE NAMA_ITEM KATEGORI IMG", oldItem, updatedItem)
        })
        ...
        it("throws 404 when item not found on put", async () => {
            mockGetItemQuery.mockResolvedValue([{ id: 1, nama: "old", kategori: "cat", updated_at: "", img_address: "" }])
            mockPutItemQuery.mockResolvedValue([])

            await expect((await itemService()).updateItem("admin", 999, "new", "cat")).rejects.toMatchObject({
                status: 404,
                message: "item tidak ditemukan"
            })
        })
    ...
    describe("removeItem", () => {
        it("deletes and creates audit log", async () => {
            const item = { id: 1, nama: "test", kategori: "cat", updated_at: "", img_address: "img.webp" }
            mockGetItemQuery.mockResolvedValue([item])
        ...
        it("throws 404 when delete returns empty", async () => {
            mockGetItemQuery.mockResolvedValue([{ id: 1, nama: "test", kategori: "cat", updated_at: "", img_address: "" }])
    ...
    describe("addImgAddress", () => {
        it("updates image and creates audit log", async () => {
            const oldItem = { id: 1, nama: "test", kategori: "cat", updated_at: "", img_address: "old.webp" }
            const newItem = { id: 1, nama: "test", kategori: "cat", updated_at: "", img_address: "new.webp" }
            mockGetItemQuery.mockResolvedValue([oldItem])
    ...
    describe("removeOldImage", () => {
        it("removes file when img_address exists", async () => {
            mockGetItemQuery.mockResolvedValue([{ id: 1, nama: "test", kategori: "cat", updated_at: "", img_address: "img.webp" }])
        ...
        it("does nothing when item has no img_address", async () => {
            mockGetItemQuery.mockResolvedValue([{ id: 1, nama: "test", kategori: "cat", updated_at: "", img_address: null as unknown as string }])
