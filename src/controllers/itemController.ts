...
import { addImgAddress, findItemById, getAllItem, insertItem, removeItem, updateItem } from "../services/itemServices";
import { z } from "zod"
...
export default function itemController() {
    return {
        ...
        putItem: async (req: Request<ReqParams, {}, ReqBody>, res: Response<ResBody>, next: NextFunction) => {
            try {
                let validate = await itemSchema.parseAsync(req.body)
        ...
        uploadItemImg: async (req: Request,res: Response<ResBody>, next: NextFunction) => {
            try {
                const data = await addImgAddress(Number(req.params.id),req.file?.filename as string)
                res.json({
                    success: true,
